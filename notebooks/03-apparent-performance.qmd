...
multi_predict_HPM <- variable.names(predict(multi_model_transformed, estimator = "HPM"))[-1] |>
  str_extract(paste(variables_to_screen, collapse = "|")) # highest probability model
multi_predict_BPM <- variable.names(predict(multi_model_transformed, estimator = "BPM"))[-1] |>
  str_extract(paste(variables_to_screen, collapse = "|")) # best predictive model, this model is weird
```
...
model_full_BPM <- logistic_reg() |>
  set_engine("glm") |>
  set_mode("classification") |>
  fit(
    reformulate(
      termlabels = multi_predict_BPM,
      response = "flag_ADR_TP_ID"
    ),
    data = data_patient_transformed_response_as_factor
  )
...
car::crPlot(model_full_BPM$fit, variable = "patient_age")
car::crPlot(model_full_BPM$fit, variable = "baseline_PLT")
```
...
data_patient_linear_check_BPM <- data_patient_transformed |>
  ...
  mutate(
    probabilities = predict(model_full_BPM$fit, data_patient_transformed, type = "response"),
    log_odds = log(probabilities / (1 - probabilities))
  ) |>
  select_if(is.numeric) |>
  gather(key = "predictors", value = "predictor_value", -log_odds, -probabilities)
...
car::influencePlot(model_full_BPM$fit)
